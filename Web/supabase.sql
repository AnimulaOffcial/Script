...
create index if not exists idx_keys_active_per_user on public.animula_keys(roblox_id, key_expires_at) where state in ('LOOT_COMPLETED','CLAIMED') and (is_unlimited or key_expires_at > now());
...
-- A row counts as "active" only if it has been LOOT_COMPLETED or CLAIMED AND its key is still valid.
-- Rows stuck in CREATED with an expired generate link (15m) do NOT block a new attempt.
...
-- RLS: enable and restrict anon to NO direct table access.
-- The backend uses the service_role key (not anon) for all writes. The anon key
-- only gets RPC access to is_key_valid and auto_expire_keys (which don't leak other rows).
alter table public.animula_keys enable row level security;
-- No policies -> anon/authenticated cannot SELECT/INSERT/UPDATE/DELETE.
-- service_role bypasses RLS, so the backend (which uses service_role) still works.
drop policy if exists "anon_all" on public.animula_keys;
drop policy if exists "anon_select" on public.animula_keys;

-- Drop dangerous blanket grants
