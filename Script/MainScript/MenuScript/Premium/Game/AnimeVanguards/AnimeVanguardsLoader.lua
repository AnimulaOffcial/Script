-- GAME MODULE: AnimeVanguards  [tier: Premium]
-- Detection: PlaceIds = { 13913892416 } | nameKeys = { "anime vanguards" }
-- REAL SYSTEMS (verified): Units, Stages, Reroll, Portals
--
-- PROMPT FITUR AnimeVanguards (buat fitur dari sistem nyata game ini, JANGAN invent):
--   - Auto-farm stage & portal
--   - Teleport & reroll
--   - Unit summon
...
--   yang tidak ada di AnimeVanguards.
local placeIds = { 13913892416 }
local nameKeys = { "anime vanguards" }
...
if IsThisGame() then
    return function(Animula, Window, Toolkit)
        local Tab = Window:MakeTab({ Name = "AnimeVanguards", Icon = "rbxassetid://4483345998" })

        Tab:AddSection({ Name = "AnimeVanguards — Premium" })
        Tab:AddParagraph("AnimeVanguards", "PlaceId: " .. tostring(game.PlaceId))
        Tab:AddButton({
            Name = "Copy PlaceId",
            Callback = function()
        ...
        Tab:AddParagraph("Real systems", "Units, Stages, Reroll, Portals")
...
-- TEMPLATE - add a REAL feature (replace with verified game logic for AnimeVanguards):
-- Tab:AddToggle({ Name = "Real Feature", Default = false, Callback = function(v)
--     -- TODO: hubungkan ke sistem nyata AnimeVanguards (lihat PROMPT di atas)
