-- GAME MODULE: Arsenal  [tier: Premium]
-- Detection: PlaceIds = { 286090429 } | nameKeys = { "arsenal" }
-- REAL SYSTEMS (verified): Guns, Kills, Rounds
--
-- PROMPT FITUR Arsenal (buat fitur dari sistem nyata game ini, JANGAN invent):
--   - ESP musuh & auto-aim (hati-hati, rawan deteksi)
--   - No-reload & auto-equip
--   - Teleport antar spawn
...
--   yang tidak ada di Arsenal.
local placeIds = { 286090429 }
local nameKeys = { "arsenal" }
...
if IsThisGame() then
    return function(Animula, Window, Toolkit)
        local Tab = Window:MakeTab({ Name = "Arsenal", Icon = "rbxassetid://4483345998" })

        Tab:AddSection({ Name = "Arsenal — Premium" })
        Tab:AddParagraph("Arsenal", "PlaceId: " .. tostring(game.PlaceId))
        Tab:AddButton({
            Name = "Copy PlaceId",
            Callback = function()
        ...
        Tab:AddParagraph("Real systems", "Guns, Kills, Rounds")
...
-- TEMPLATE - add a REAL feature (replace with verified game logic for Arsenal):
-- Tab:AddToggle({ Name = "Real Feature", Default = false, Callback = function(v)
--     -- TODO: hubungkan ke sistem nyata Arsenal (lihat PROMPT di atas)
