-- GAME MODULE: Piggy  [tier: Premium]
-- Detection: PlaceIds = { 4623386078 } | nameKeys = { "piggy" }
-- REAL SYSTEMS (verified): Maps, Piggy, Survival, Items
--
-- PROMPT FITUR Piggy (buat fitur dari sistem nyata game ini, JANGAN invent):
--   - ESP piggy & item
--   - Auto-win map (via UI tombol)
--   - Teleport antar room
...
--   yang tidak ada di Piggy.
local placeIds = { 4623386078 }
local nameKeys = { "piggy" }
...
if IsThisGame() then
    return function(Animula, Window, Toolkit)
        local Tab = Window:MakeTab({ Name = "Piggy", Icon = "rbxassetid://4483345998" })

        Tab:AddSection({ Name = "Piggy — Premium" })
        Tab:AddParagraph("Piggy", "PlaceId: " .. tostring(game.PlaceId))
        Tab:AddButton({
            Name = "Copy PlaceId",
            Callback = function()
        ...
        Tab:AddParagraph("Real systems", "Maps, Piggy, Survival, Items")
...
-- TEMPLATE - add a REAL feature (replace with verified game logic for Piggy):
-- Tab:AddToggle({ Name = "Real Feature", Default = false, Callback = function(v)
--     -- TODO: hubungkan ke sistem nyata Piggy (lihat PROMPT di atas)
