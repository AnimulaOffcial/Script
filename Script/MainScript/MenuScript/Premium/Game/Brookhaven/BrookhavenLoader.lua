-- GAME MODULE: Brookhaven  [tier: Premium]
-- Detection: PlaceIds = { 4924922222 } | nameKeys = { "brookhaven" }
-- REAL SYSTEMS (verified): Roleplay, Build/Houses, Money/Economy
--
-- PROMPT FITUR Brookhaven (buat fitur dari sistem nyata game ini, JANGAN invent):
--   - Teleport ke rumah/plot teman
--   - Spawn & kendalikan kendaraan (mobil, helikopter, perahu)
--   - Mode build bebas & anti-Ragdoll
...
--   yang tidak ada di Brookhaven.
local placeIds = { 4924922222 }
local nameKeys = { "brookhaven" }
...
if IsThisGame() then
    return function(Animula, Window, Toolkit)
        local Tab = Window:MakeTab({ Name = "Brookhaven", Icon = "rbxassetid://4483345998" })

        Tab:AddSection({ Name = "Brookhaven — Premium" })
        Tab:AddParagraph("Brookhaven", "PlaceId: " .. tostring(game.PlaceId))
        Tab:AddButton({
            Name = "Copy PlaceId",
            Callback = function()
        ...
        Tab:AddParagraph("Real systems", "Roleplay, Build/Houses, Money/Economy")
...
-- TEMPLATE - add a REAL feature (replace with verified game logic for Brookhaven):
-- Tab:AddToggle({ Name = "Real Feature", Default = false, Callback = function(v)
--     -- TODO: hubungkan ke sistem nyata Brookhaven (lihat PROMPT di atas)
