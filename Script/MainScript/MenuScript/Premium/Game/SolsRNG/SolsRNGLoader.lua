-- GAME MODULE: SolsRNG  [tier: Premium]
-- Detection: PlaceIds = { 16732694052 } | nameKeys = { "sol's rng", "sols rng" }
-- REAL SYSTEMS (verified): Rolls, Auras, Biomes, Potions
--
-- PROMPT FITUR SolsRNG (buat fitur dari sistem nyata game ini, JANGAN invent):
--   - Auto-roll & biome teleport
--   - ESP aura rarity & potion
--   - Breakables farm
...
--   yang tidak ada di SolsRNG.
local placeIds = { 16732694052 }
local nameKeys = { "sol's rng", "sols rng" }
...
if IsThisGame() then
    return function(Animula, Window, Toolkit)
        local Tab = Window:MakeTab({ Name = "SolsRNG", Icon = "rbxassetid://4483345998" })

        Tab:AddSection({ Name = "SolsRNG — Premium" })
        Tab:AddParagraph("SolsRNG", "PlaceId: " .. tostring(game.PlaceId))
        Tab:AddButton({
            Name = "Copy PlaceId",
            Callback = function()
        ...
        Tab:AddParagraph("Real systems", "Rolls, Auras, Biomes, Potions")
...
-- TEMPLATE - add a REAL feature (replace with verified game logic for SolsRNG):
-- Tab:AddToggle({ Name = "Real Feature", Default = false, Callback = function(v)
--     -- TODO: hubungkan ke sistem nyata SolsRNG (lihat PROMPT di atas)
