-- GAME MODULE: MurderMystery2  [tier: Premium]
-- Detection: PlaceIds = { 142823291 } | nameKeys = { "murder mystery 2" }
-- REAL SYSTEMS (verified): Sheriff/Murderer, Coins, Roles
--
-- PROMPT FITUR MurderMystery2 (buat fitur dari sistem nyata game ini, JANGAN invent):
--   - ESP peran (Sheriff/Murderer/Innocent)
--   - Teleport & auto-grab gun
--   - Coin farm
...
--   yang tidak ada di MurderMystery2.
local placeIds = { 142823291 }
local nameKeys = { "murder mystery 2" }
...
if IsThisGame() then
    return function(Animula, Window, Toolkit)
        local Tab = Window:MakeTab({ Name = "MurderMystery2", Icon = "rbxassetid://4483345998" })

        Tab:AddSection({ Name = "MurderMystery2 — Premium" })
        Tab:AddParagraph("MurderMystery2", "PlaceId: " .. tostring(game.PlaceId))
        Tab:AddButton({
            Name = "Copy PlaceId",
            Callback = function()
        ...
        Tab:AddParagraph("Real systems", "Sheriff/Murderer, Coins, Roles")
...
-- TEMPLATE - add a REAL feature (replace with verified game logic for MurderMystery2):
-- Tab:AddToggle({ Name = "Real Feature", Default = false, Callback = function(v)
--     -- TODO: hubungkan ke sistem nyata MurderMystery2 (lihat PROMPT di atas)
