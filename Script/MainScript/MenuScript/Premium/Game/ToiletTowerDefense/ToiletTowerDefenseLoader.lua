-- GAME MODULE: ToiletTowerDefense  [tier: Premium]
-- Detection: PlaceIds = { 14661121985 } | nameKeys = { "toilet tower defense" }
-- REAL SYSTEMS (verified): Toilets, Units, Waves, Scientist
--
-- PROMPT FITUR ToiletTowerDefense (buat fitur dari sistem nyata game ini, JANGAN invent):
--   - Auto-place & upgrade unit
--   - Teleport & coin farm
--   - Skip wave
...
--   yang tidak ada di ToiletTowerDefense.
local placeIds = { 14661121985 }
local nameKeys = { "toilet tower defense" }
...
if IsThisGame() then
    return function(Animula, Window, Toolkit)
        local Tab = Window:MakeTab({ Name = "ToiletTowerDefense", Icon = "rbxassetid://4483345998" })

        Tab:AddSection({ Name = "ToiletTowerDefense — Premium" })
        Tab:AddParagraph("ToiletTowerDefense", "PlaceId: " .. tostring(game.PlaceId))
        Tab:AddButton({
            Name = "Copy PlaceId",
            Callback = function()
        ...
        Tab:AddParagraph("Real systems", "Toilets, Units, Waves, Scientist")
...
-- TEMPLATE - add a REAL feature (replace with verified game logic for ToiletTowerDefense):
-- Tab:AddToggle({ Name = "Real Feature", Default = false, Callback = function(v)
--     -- TODO: hubungkan ke sistem nyata ToiletTowerDefense (lihat PROMPT di atas)
