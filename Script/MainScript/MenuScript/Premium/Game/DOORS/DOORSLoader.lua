-- GAME MODULE: DOORS  [tier: Premium]
-- Detection: PlaceIds = { 651614604 } | nameKeys = { "doors" }
-- REAL SYSTEMS (verified): Floors, Entities, Items, Rooms
--
-- PROMPT FITUR DOORS (buat fitur dari sistem nyata game ini, JANGAN invent):
--   - ESP entity & item highlight
--   - Auto-open door & floor skip
--   - No-damage
...
--   yang tidak ada di DOORS.
local placeIds = { 651614604 }
local nameKeys = { "doors" }
...
if IsThisGame() then
    return function(Animula, Window, Toolkit)
        local Tab = Window:MakeTab({ Name = "DOORS", Icon = "rbxassetid://4483345998" })

        Tab:AddSection({ Name = "DOORS — Premium" })
        Tab:AddParagraph("DOORS", "PlaceId: " .. tostring(game.PlaceId))
        Tab:AddButton({
            Name = "Copy PlaceId",
            Callback = function()
        ...
        Tab:AddParagraph("Real systems", "Floors, Entities, Items, Rooms")
...
-- TEMPLATE - add a REAL feature (replace with verified game logic for DOORS):
-- Tab:AddToggle({ Name = "Real Feature", Default = false, Callback = function(v)
--     -- TODO: hubungkan ke sistem nyata DOORS (lihat PROMPT di atas)
