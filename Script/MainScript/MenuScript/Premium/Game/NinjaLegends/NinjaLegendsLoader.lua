-- GAME MODULE: NinjaLegends  [tier: Premium]
-- Detection: PlaceIds = { 2080491456 } | nameKeys = { "ninja legends" }
-- REAL SYSTEMS (verified): Swords, Chi, Sells, Ranks
--
-- PROMPT FITUR NinjaLegends (buat fitur dari sistem nyata game ini, JANGAN invent):
--   - Auto-swing & chi farm
--   - Teleport & auto-sell
--   - Rank up
...
--   yang tidak ada di NinjaLegends.
local placeIds = { 2080491456 }
local nameKeys = { "ninja legends" }
...
if IsThisGame() then
    return function(Animula, Window, Toolkit)
        local Tab = Window:MakeTab({ Name = "NinjaLegends", Icon = "rbxassetid://4483345998" })

        Tab:AddSection({ Name = "NinjaLegends — Premium" })
        Tab:AddParagraph("NinjaLegends", "PlaceId: " .. tostring(game.PlaceId))
        Tab:AddButton({
            Name = "Copy PlaceId",
            Callback = function()
        ...
        Tab:AddParagraph("Real systems", "Swords, Chi, Sells, Ranks")
...
-- TEMPLATE - add a REAL feature (replace with verified game logic for NinjaLegends):
-- Tab:AddToggle({ Name = "Real Feature", Default = false, Callback = function(v)
--     -- TODO: hubungkan ke sistem nyata NinjaLegends (lihat PROMPT di atas)
