-- GAME MODULE: WelcomeToBloxburg  [tier: Premium]
-- Detection: PlaceIds = { 185655149 } | nameKeys = { "bloxburg" }
-- REAL SYSTEMS (verified): Build/Houses, Jobs, Money, Vehicles
--
-- PROMPT FITUR WelcomeToBloxburg (buat fitur dari sistem nyata game ini, JANGAN invent):
--   - Money farm via job (via UI tombol)
--   - Teleport ke plot & build
--   - Kendaraan spawn
...
--   yang tidak ada di WelcomeToBloxburg.
local placeIds = { 185655149 }
local nameKeys = { "bloxburg" }
...
if IsThisGame() then
    return function(Animula, Window, Toolkit)
        local Tab = Window:MakeTab({ Name = "WelcomeToBloxburg", Icon = "rbxassetid://4483345998" })

        Tab:AddSection({ Name = "WelcomeToBloxburg — Premium" })
        Tab:AddParagraph("WelcomeToBloxburg", "PlaceId: " .. tostring(game.PlaceId))
        Tab:AddButton({
            Name = "Copy PlaceId",
            Callback = function()
        ...
        Tab:AddParagraph("Real systems", "Build/Houses, Jobs, Money, Vehicles")
...
-- TEMPLATE - add a REAL feature (replace with verified game logic for WelcomeToBloxburg):
-- Tab:AddToggle({ Name = "Real Feature", Default = false, Callback = function(v)
--     -- TODO: hubungkan ke sistem nyata WelcomeToBloxburg (lihat PROMPT di atas)
