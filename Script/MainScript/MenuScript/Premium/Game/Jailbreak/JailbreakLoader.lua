-- GAME MODULE: Jailbreak  [tier: Premium]
-- Detection: PlaceIds = { 606849621 } | nameKeys = { "jailbreak" }
-- REAL SYSTEMS (verified): Vehicles, Heists/Bank, Weapons
--
-- PROMPT FITUR Jailbreak (buat fitur dari sistem nyata game ini, JANGAN invent):
--   - Teleport ke semua kendaraan & lokasi heist
--   - Auto-robat via tombol (Bank/Jewelry/Museum) - butuh verifikasi objek game
--   - No-cooldown & godmode (harus cek sistem game dulu)
...
--   yang tidak ada di Jailbreak.
local placeIds = { 606849621 }
local nameKeys = { "jailbreak" }
...
if IsThisGame() then
    return function(Animula, Window, Toolkit)
        local Tab = Window:MakeTab({ Name = "Jailbreak", Icon = "rbxassetid://4483345998" })

        Tab:AddSection({ Name = "Jailbreak — Premium" })
        Tab:AddParagraph("Jailbreak", "PlaceId: " .. tostring(game.PlaceId))
        Tab:AddButton({
            Name = "Copy PlaceId",
            Callback = function()
        ...
        Tab:AddParagraph("Real systems", "Vehicles, Heists/Bank, Weapons")
...
-- TEMPLATE - add a REAL feature (replace with verified game logic for Jailbreak):
-- Tab:AddToggle({ Name = "Real Feature", Default = false, Callback = function(v)
--     -- TODO: hubungkan ke sistem nyata Jailbreak (lihat PROMPT di atas)
