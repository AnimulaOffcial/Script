-- GAME MODULE: BedWars  [tier: Premium]
-- Detection: PlaceIds = { 2625602616 } | nameKeys = { "bedwars" }
-- REAL SYSTEMS (verified): Beds, Resources, Kits, Weapons
--
-- PROMPT FITUR BedWars (buat fitur dari sistem nyata game ini, JANGAN invent):
--   - Auto-collect resource (iron/gold/diamond)
--   - Bed ESP & protect
--   - Kit auto & teleport
...
--   yang tidak ada di BedWars.
local placeIds = { 2625602616 }
local nameKeys = { "bedwars" }
...
if IsThisGame() then
    return function(Animula, Window, Toolkit)
        local Tab = Window:MakeTab({ Name = "BedWars", Icon = "rbxassetid://4483345998" })

        Tab:AddSection({ Name = "BedWars — Premium" })
        Tab:AddParagraph("BedWars", "PlaceId: " .. tostring(game.PlaceId))
        Tab:AddButton({
            Name = "Copy PlaceId",
            Callback = function()
        ...
        Tab:AddParagraph("Real systems", "Beds, Resources, Kits, Weapons")
...
-- TEMPLATE - add a REAL feature (replace with verified game logic for BedWars):
-- Tab:AddToggle({ Name = "Real Feature", Default = false, Callback = function(v)
--     -- TODO: hubungkan ke sistem nyata BedWars (lihat PROMPT di atas)
