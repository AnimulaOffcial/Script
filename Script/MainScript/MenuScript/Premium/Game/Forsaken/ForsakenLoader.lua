-- GAME MODULE: Forsaken  [tier: Premium]
-- Detection: PlaceIds = {  } | nameKeys = { "forsaken" }
-- REAL SYSTEMS (verified): Survivor, Killer, Rounds, Abilities
--
-- PROMPT FITUR Forsaken (buat fitur dari sistem nyata game ini, JANGAN invent):
--   - ESP killer & auto-survive
--   - Ability auto & teleport
--   - Round win
...
--   yang tidak ada di Forsaken.
local placeIds = {  }
local nameKeys = { "forsaken" }
...
if IsThisGame() then
    return function(Animula, Window, Toolkit)
        local Tab = Window:MakeTab({ Name = "Forsaken", Icon = "rbxassetid://4483345998" })

        Tab:AddSection({ Name = "Forsaken — Premium" })
        Tab:AddParagraph("Forsaken", "PlaceId: " .. tostring(game.PlaceId))
        Tab:AddButton({
            Name = "Copy PlaceId",
            Callback = function()
        ...
        Tab:AddParagraph("Real systems", "Survivor, Killer, Rounds, Abilities")
...
-- TEMPLATE - add a REAL feature (replace with verified game logic for Forsaken):
-- Tab:AddToggle({ Name = "Real Feature", Default = false, Callback = function(v)
--     -- TODO: hubungkan ke sistem nyata Forsaken (lihat PROMPT di atas)
