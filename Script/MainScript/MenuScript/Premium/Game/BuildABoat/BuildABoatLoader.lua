-- GAME MODULE: BuildABoat  [tier: Premium]
-- Detection: PlaceIds = { 880399819 } | nameKeys = { "build a boat" }
-- REAL SYSTEMS (verified): Blocks, Boats, Gold, Quest
--
-- PROMPT FITUR BuildABoat (buat fitur dari sistem nyata game ini, JANGAN invent):
--   - Auto-build & teleport block
--   - Gold farm & quest teleport
--   - Boost kecepatan boat
...
--   yang tidak ada di BuildABoat.
local placeIds = { 880399819 }
local nameKeys = { "build a boat" }
...
if IsThisGame() then
    return function(Animula, Window, Toolkit)
        local Tab = Window:MakeTab({ Name = "BuildABoat", Icon = "rbxassetid://4483345998" })

        Tab:AddSection({ Name = "BuildABoat — Premium" })
        Tab:AddParagraph("BuildABoat", "PlaceId: " .. tostring(game.PlaceId))
        Tab:AddButton({
            Name = "Copy PlaceId",
            Callback = function()
        ...
        Tab:AddParagraph("Real systems", "Blocks, Boats, Gold, Quest")
...
-- TEMPLATE - add a REAL feature (replace with verified game logic for BuildABoat):
-- Tab:AddToggle({ Name = "Real Feature", Default = false, Callback = function(v)
--     -- TODO: hubungkan ke sistem nyata BuildABoat (lihat PROMPT di atas)
