-- GAME MODULE: BreakIn  [tier: Premium]
-- Detection: PlaceIds = { 1465373301 } | nameKeys = { "break in" }
-- REAL SYSTEMS (verified): Stages, Money, Stealth, Survival
--
-- PROMPT FITUR BreakIn (buat fitur dari sistem nyata game ini, JANGAN invent):
--   - ESP enemy & teleport
--   - Auto-win stage (via UI tombol)
--   - Money farm
...
--   yang tidak ada di BreakIn.
local placeIds = { 1465373301 }
local nameKeys = { "break in" }
...
if IsThisGame() then
    return function(Animula, Window, Toolkit)
        local Tab = Window:MakeTab({ Name = "BreakIn", Icon = "rbxassetid://4483345998" })

        Tab:AddSection({ Name = "BreakIn — Premium" })
        Tab:AddParagraph("BreakIn", "PlaceId: " .. tostring(game.PlaceId))
        Tab:AddButton({
            Name = "Copy PlaceId",
            Callback = function()
        ...
        Tab:AddParagraph("Real systems", "Stages, Money, Stealth, Survival")
...
-- TEMPLATE - add a REAL feature (replace with verified game logic for BreakIn):
-- Tab:AddToggle({ Name = "Real Feature", Default = false, Callback = function(v)
--     -- TODO: hubungkan ke sistem nyata BreakIn (lihat PROMPT di atas)
