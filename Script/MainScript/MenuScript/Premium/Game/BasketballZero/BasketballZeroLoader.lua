-- GAME MODULE: BasketballZero  [tier: Premium]
-- Detection: PlaceIds = {  } | nameKeys = { "basketball zero" }
-- REAL SYSTEMS (verified): Styles, Dunks, Ranked, Flow
--
-- PROMPT FITUR BasketballZero (buat fitur dari sistem nyata game ini, JANGAN invent):
--   - Auto-dunk & ESP
--   - Style & flow
--   - Ranked farm
...
--   yang tidak ada di BasketballZero.
local placeIds = {  }
local nameKeys = { "basketball zero" }
...
if IsThisGame() then
    return function(Animula, Window, Toolkit)
        local Tab = Window:MakeTab({ Name = "BasketballZero", Icon = "rbxassetid://4483345998" })

        Tab:AddSection({ Name = "BasketballZero — Premium" })
        Tab:AddParagraph("BasketballZero", "PlaceId: " .. tostring(game.PlaceId))
        Tab:AddButton({
            Name = "Copy PlaceId",
            Callback = function()
        ...
        Tab:AddParagraph("Real systems", "Styles, Dunks, Ranked, Flow")
...
-- TEMPLATE - add a REAL feature (replace with verified game logic for BasketballZero):
-- Tab:AddToggle({ Name = "Real Feature", Default = false, Callback = function(v)
--     -- TODO: hubungkan ke sistem nyata BasketballZero (lihat PROMPT di atas)
