-- GAME MODULE: BubbleGumSimulatorInfinity  [tier: Premium]
-- Detection: PlaceIds = { 125558926610 } | nameKeys = { "bubble gum simulator" }
-- REAL SYSTEMS (verified): Coins, Hatch, Worlds, Stats
--
-- PROMPT FITUR BubbleGumSimulatorInfinity (buat fitur dari sistem nyata game ini, JANGAN invent):
--   - Auto-hatch & world teleport
--   - Coin farm & stat
--   - Pet equip
...
--   yang tidak ada di BubbleGumSimulatorInfinity.
local placeIds = { 125558926610 }
local nameKeys = { "bubble gum simulator" }
...
if IsThisGame() then
    return function(Animula, Window, Toolkit)
        local Tab = Window:MakeTab({ Name = "BubbleGumSimulatorInfinity", Icon = "rbxassetid://4483345998" })

        Tab:AddSection({ Name = "BubbleGumSimulatorInfinity — Premium" })
        Tab:AddParagraph("BubbleGumSimulatorInfinity", "PlaceId: " .. tostring(game.PlaceId))
        Tab:AddButton({
            Name = "Copy PlaceId",
            Callback = function()
        ...
        Tab:AddParagraph("Real systems", "Coins, Hatch, Worlds, Stats")
...
-- TEMPLATE - add a REAL feature (replace with verified game logic for BubbleGumSimulatorInfinity):
-- Tab:AddToggle({ Name = "Real Feature", Default = false, Callback = function(v)
--     -- TODO: hubungkan ke sistem nyata BubbleGumSimulatorInfinity (lihat PROMPT di atas)
