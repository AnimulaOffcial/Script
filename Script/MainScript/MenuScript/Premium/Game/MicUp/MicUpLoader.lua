-- GAME MODULE: MicUp  [tier: Premium]
-- Detection: PlaceIds = { 3714313133 } | nameKeys = { "mic up" }
-- REAL SYSTEMS (verified): Voice, Roblox VC, Hangout
--
-- PROMPT FITUR MicUp (buat fitur dari sistem nyata game ini, JANGAN invent):
--   - ESP player & teleport
--   - No-clip stage
--   - Follow player
...
--   yang tidak ada di MicUp.
local placeIds = { 3714313133 }
local nameKeys = { "mic up" }
...
if IsThisGame() then
    return function(Animula, Window, Toolkit)
        local Tab = Window:MakeTab({ Name = "MicUp", Icon = "rbxassetid://4483345998" })

        Tab:AddSection({ Name = "MicUp — Premium" })
        Tab:AddParagraph("MicUp", "PlaceId: " .. tostring(game.PlaceId))
        Tab:AddButton({
            Name = "Copy PlaceId",
            Callback = function()
        ...
        Tab:AddParagraph("Real systems", "Voice, Roblox VC, Hangout")
...
-- TEMPLATE - add a REAL feature (replace with verified game logic for MicUp):
-- Tab:AddToggle({ Name = "Real Feature", Default = false, Callback = function(v)
--     -- TODO: hubungkan ke sistem nyata MicUp (lihat PROMPT di atas)
