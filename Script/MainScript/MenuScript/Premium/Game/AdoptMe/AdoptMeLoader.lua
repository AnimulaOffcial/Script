-- GAME MODULE: AdoptMe  [tier: Premium]
-- Detection: PlaceIds = { 920587237 } | nameKeys = { "adopt me" }
-- REAL SYSTEMS (verified): Pets, Eggs, Trading, Money
--
-- PROMPT FITUR AdoptMe (buat fitur dari sistem nyata game ini, JANGAN invent):
--   - Auto-collect pet & age boost
--   - Teleport pet / task
--   - Auto-hatch egg (via UI tombol)
...
--   yang tidak ada di AdoptMe.
local placeIds = { 920587237 }
local nameKeys = { "adopt me" }
...
if IsThisGame() then
    return function(Animula, Window, Toolkit)
        local Tab = Window:MakeTab({ Name = "AdoptMe", Icon = "rbxassetid://4483345998" })

        Tab:AddSection({ Name = "AdoptMe — Premium" })
        Tab:AddParagraph("AdoptMe", "PlaceId: " .. tostring(game.PlaceId))
        Tab:AddButton({
            Name = "Copy PlaceId",
            Callback = function()
        ...
        Tab:AddParagraph("Real systems", "Pets, Eggs, Trading, Money")
...
-- TEMPLATE - add a REAL feature (replace with verified game logic for AdoptMe):
-- Tab:AddToggle({ Name = "Real Feature", Default = false, Callback = function(v)
--     -- TODO: hubungkan ke sistem nyata AdoptMe (lihat PROMPT di atas)
