-- GAME MODULE: ShindoLife  [tier: Premium]
-- Detection: PlaceIds = { 4777817885 } | nameKeys = { "shindo life" }
-- REAL SYSTEMS (verified): Breaths, Spins, Rank, Elements, Modes
--
-- PROMPT FITUR ShindoLife (buat fitur dari sistem nyata game ini, JANGAN invent):
--   - Auto-spin & stat
...
--   yang tidak ada di ShindoLife.
local placeIds = { 4777817885 }
local nameKeys = { "shindo life" }
...
if IsThisGame() then
    return function(Animula, Window, Toolkit)
        local Tab = Window:MakeTab({ Name = "ShindoLife", Icon = "rbxassetid://4483345998" })

        Tab:AddSection({ Name = "ShindoLife — Premium" })
        Tab:AddParagraph("ShindoLife", "PlaceId: " .. tostring(game.PlaceId))
        Tab:AddButton({
            Name = "Copy PlaceId",
            Callback = function()
        ...
        Tab:AddParagraph("Real systems", "Breaths, Spins, Rank, Elements, Modes")
...
-- TEMPLATE - add a REAL feature (replace with verified game logic for ShindoLife):
-- Tab:AddToggle({ Name = "Real Feature", Default = false, Callback = function(v)
--     -- TODO: hubungkan ke sistem nyata ShindoLife (lihat PROMPT di atas)
