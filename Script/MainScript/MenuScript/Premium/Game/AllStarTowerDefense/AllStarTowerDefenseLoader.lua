-- GAME MODULE: AllStarTowerDefense  [tier: Premium]
-- Detection: PlaceIds = { 5972744278 } | nameKeys = { "all star tower defense" }
-- REAL SYSTEMS (verified): Units, Story, Coins, Summon
--
-- PROMPT FITUR AllStarTowerDefense (buat fitur dari sistem nyata game ini, JANGAN invent):
--   - Auto-summon & place unit
--   - Teleport & coin farm
--   - Auto-story
...
--   yang tidak ada di AllStarTowerDefense.
local placeIds = { 5972744278 }
local nameKeys = { "all star tower defense" }
...
if IsThisGame() then
    return function(Animula, Window, Toolkit)
        local Tab = Window:MakeTab({ Name = "AllStarTowerDefense", Icon = "rbxassetid://4483345998" })

        Tab:AddSection({ Name = "AllStarTowerDefense — Premium" })
        Tab:AddParagraph("AllStarTowerDefense", "PlaceId: " .. tostring(game.PlaceId))
        Tab:AddButton({
            Name = "Copy PlaceId",
            Callback = function()
        ...
        Tab:AddParagraph("Real systems", "Units, Story, Coins, Summon")
...
-- TEMPLATE - add a REAL feature (replace with verified game logic for AllStarTowerDefense):
-- Tab:AddToggle({ Name = "Real Feature", Default = false, Callback = function(v)
--     -- TODO: hubungkan ke sistem nyata AllStarTowerDefense (lihat PROMPT di atas)
