-- GAME MODULE: BeeSwarmSimulator  [tier: Premium]
-- Detection: PlaceIds = { 1537690962 } | nameKeys = { "bee swarm" }
-- REAL SYSTEMS (verified): Bees, Pollen, Fields, Honey
--
-- PROMPT FITUR BeeSwarmSimulator (buat fitur dari sistem nyata game ini, JANGAN invent):
--   - Auto-collect pollen & honey
--   - Teleport antar field
--   - Auto-buy & bee farm
...
--   yang tidak ada di BeeSwarmSimulator.
local placeIds = { 1537690962 }
local nameKeys = { "bee swarm" }
...
if IsThisGame() then
    return function(Animula, Window, Toolkit)
        local Tab = Window:MakeTab({ Name = "BeeSwarmSimulator", Icon = "rbxassetid://4483345998" })

        Tab:AddSection({ Name = "BeeSwarmSimulator — Premium" })
        Tab:AddParagraph("BeeSwarmSimulator", "PlaceId: " .. tostring(game.PlaceId))
        Tab:AddButton({
            Name = "Copy PlaceId",
            Callback = function()
        ...
        Tab:AddParagraph("Real systems", "Bees, Pollen, Fields, Honey")
...
-- TEMPLATE - add a REAL feature (replace with verified game logic for BeeSwarmSimulator):
-- Tab:AddToggle({ Name = "Real Feature", Default = false, Callback = function(v)
--     -- TODO: hubungkan ke sistem nyata BeeSwarmSimulator (lihat PROMPT di atas)
